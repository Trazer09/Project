...
const page = () => {
  ...
  const [mainTask, setMainTask] = useState([]); // storing the tasks and description using array
      ...
      return (
        <ul className='flex items-center justify-between' key={i}>
          ...
          <button
            ...
            className='bg-red-400 text-white font-bold rounded p-2'
          >
            DELETE
          </button>
        </ul>
  ...
  return (
    <>
      ...
      <form onSubmit={submitHandler} className='flex justify-center'>
        <input
          className='border-4 border-black rounded m-8 px-6 py-1'
      ...
      <hr />
      <br />
      <div className='bg-gray-300 p-7'>{renderTask}</div>
    </>
  );
};
